...
import { UnusedTreeProvider, DependencyTreeProvider } from './treeProviders';
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const graphWebview = new GraphWebviewProvider(context.extensionUri);
  const unusedTree = new UnusedTreeProvider();
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('sfDependencyAnalysis.showGraph', async () => {
  ...
  // ─── Command: Refresh ───────────────────────────────────
  context.subscriptions.push(
    vscode.commands.registerCommand('sfDependencyAnalysis.refresh', () => {
  ...
  // ─── Command: Find Unused ───────────────────────────────
  context.subscriptions.push(
    vscode.commands.registerCommand('sfDependencyAnalysis.findUnused', async () => {
      if (!analysisResult) {
        // Run analysis first
  ...
  // ─── Command: Focus Node ────────────────────────────────
  context.subscriptions.push(
    vscode.commands.registerCommand('sfDependencyAnalysis.focusNode', async () => {
      if (!analysisResult) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('sfDependencyAnalysis.impactAnalysis', async () => {
      if (!analysisResult) {
        vscode.window.showWarningMessage('Run the dependency analysis first.');
        return;
      }
      ...
      if (impacted.length === 0) {
        vscode.window.showInformationMessage(
          `No other classes or components depend on "${selected.label.replace(/\$\([^)]+\)\s*/, '')}".`
        );
        return;
      }
      ...
      // Group by depth
      const depthMap = new Map<number, typeof impacted>();
  ...
  // ─── Command: Find Cycles ──────────────────────────────
  context.subscriptions.push(
    vscode.commands.registerCommand('sfDependencyAnalysis.findCycles', async () => {
      if (!analysisResult) {
        vscode.window.showWarningMessage('Run the dependency analysis first.');
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('sfDependencyAnalysis.exportReport', async () => {
      if (!analysisResult) {
        vscode.window.showWarningMessage('Run the dependency analysis first.');
        return;
      }
      ...
      const report = {
        ...
        cycles: analyzer.findCycles(analysisResult.graph).map(cycle =>
          cycle.map(id => analysisResult!.graph.nodes.get(id)?.name ?? id)
        ),
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('sfDependencyAnalysis._updateDiagnostics', () => {
      diagnosticCollection.clear();
      if (!analysisResult) { return; }

      for (const item of analysisResult.unused) {
        if (item.confidence === 'low') { continue; } // skip low confidence
