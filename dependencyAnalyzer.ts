...
import {
  ...
  AnalysisResult, UnusedItem, ParseError,
} from './types';
import { ApexParser } from './apexParser';
import { LwcParser } from './lwcParser';
import { TriggerParser } from './triggerParser';
import { AuraParser } from './auraParser';
...
export class DependencyAnalyzer {
  ...
  private triggerParser = new TriggerParser();
  ...
  async analyze(
    ...
    for (let i = 0; i < apexFiles.length; i++) {
      try {
        ...
        if (!includeTests && node.metadata.isTest) { continue; }
    ...
    for (const item of unused) {
      ...
      if (node) { node.isUnused = true; }
    }

    errors.push(
      ...this.apexParser.getErrors(),
      ...this.lwcParser.getErrors(),
      ...this.triggerParser.getErrors(),
      ...this.auraParser.getErrors(),
    );
   ...
   */
  getNeighbors(graph: DependencyGraph, nodeId: string): {
    dependsOn: Array<{ node: DependencyNode; edgeType: string }>;
    usedBy: Array<{ node: DependencyNode; edgeType: string }>;
  } {
    const dependsOn: Array<{ node: DependencyNode; edgeType: string }> = [];
    const usedBy: Array<{ node: DependencyNode; edgeType: string }> = [];

    for (const edge of graph.edges) {
      if (edge.source === nodeId) {
        const target = graph.nodes.get(edge.target);
        if (target) { dependsOn.push({ node: target, edgeType: edge.type }); }
      }
      if (edge.target === nodeId) {
        const source = graph.nodes.get(edge.source);
        if (source) { usedBy.push({ node: source, edgeType: edge.type }); }
      }
    }
  ...
  ): Map<string, { node: DependencyNode; depth: number }> {
    const result = new Map<string, { node: DependencyNode; depth: number }>();
   ...
   */
  getImpactedNodes(graph: DependencyGraph, nodeId: string): DependencyNode[] {
   ...
   */
  findClusters(graph: DependencyGraph): Map<number, DependencyNode[]> {
    const visited = new Set<string>();
    const clusters = new Map<number, DependencyNode[]>();
    let clusterIndex = 0;
   ...
   */
  getNodeMetrics(graph: DependencyGraph, nodeId: string): {
    inDegree: number;
    outDegree: number;
    isHub: boolean;
    isBridge: boolean;
  ...
  private findUnused(graph: DependencyGraph): UnusedItem[] {
    ...
    for (const [id, node] of graph.nodes) {
      ...
      if (userEntryPoints.has(id)) { continue; }
  ...
  private isEntryPoint(node: DependencyNode): string | null {
    ...
    if (node.type === 'aura' && meta.interfaces && meta.interfaces.length > 0) {
      const pageInterfaces = meta.interfaces.some(i =>
        i.includes('flexipage') || i.includes('appHostable') ||
  ...
  private assessConfidence(node: DependencyNode): 'high' | 'medium' | 'low' {
    const meta = node.metadata;

    if (
      node.type === 'apex-class' &&
      !meta.isAbstract &&
      !meta.isVirtual &&
    ...
    if (meta.isAbstract || meta.isVirtual) { return 'low'; }
    if (node.type === 'apex-interface') { return 'low'; }

    return 'medium';
  }

  private buildReason(node: DependencyNode): string {
